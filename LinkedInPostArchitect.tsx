...
import { 
  Linkedin, Sparkles, Loader2, Copy, Check, Zap, 
...
export const LinkedInPostArchitect: React.FC = () => {
  ...
  const langT = t.tools['linkedin-post'] || {
  ...
  const generatePost = async () => {
    ...
    try {
      const response = await ai.models.generateContent({
        ...
        contents: `Write a high-impact, professional LinkedIn thought leadership post about: "${topic}". Use strategic bullet points and a clear professional call to action. Use standard numerals 0123456789 for any data points. Tone should be executive and authoritative. Return only the final post content.`,
      });
      ...
      <div className="bg-[#0a0a0a] border border-blue-600/30 rounded-[3rem] p-8 max-w-6xl mx-auto shadow-2xl relative overflow-hidden selection:bg-blue-600 selection:text-white">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[9px] font-bold text-blue-500/40 uppercase tracking-[0.4em]">Executive Narrative Synthesis Node</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-5 space-y-8">
            <div className="space-y-4">
              <label className="text-[10px] font-black uppercase tracking-[0.4em] text-gray-500 italic px-2">Thought Coordinate Ingestion</label>
              <textarea
                value={topic}
                onChange={(e) => setTopic(e.target.value)}
                className="w-full h-48 bg-black border border-white/5 rounded-2xl p-6 text-white text-sm outline-none focus:border-blue-600/40 transition-all placeholder-white/5 resize-none shadow-inner"
                placeholder="What industry insight or professional milestone are you sharing?"
              />
            </div>
            ...
            <div className="relative flex-grow bg-black/60 border border-white/5 rounded-[3.5rem] p-10 flex flex-col min-h-[450px] overflow-hidden group shadow-inner">
               ...
               {loading ? (
                 <div className="h-full flex flex-col items-center justify-center space-y-6 animate-pulse">
                    <Linkedin size={64} className="text-blue-500" />
                    <p className="text-[10px] font-black uppercase tracking-[0.5em] text-center">Synthesizing Executive Authority</p>
                 </div>
               ) : output ? (
                 ...
                 <div className="h-full flex flex-col items-center justify-center opacity-10 space-y-6">
                   <Linkedin size={100} className="mx-auto" />
                   <p className="text-[10px] font-black uppercase tracking-[0.5em]">Awaiting Professional Input</p>
                 </div>
               )}
            </div>
          </div>
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-2 gap-6 opacity-40">
           <div className="flex items-center gap-4 p-4 bg-white/5 rounded-2xl border border-white/5">
             ...
             <p className="text-[9px] font-black uppercase tracking-widest leading-loose italic">Neural Thought Leadership Logic Active</p>
           </div>
           <div className="flex items-center gap-4 p-4 bg-white/5 rounded-2xl border border-white/5">
