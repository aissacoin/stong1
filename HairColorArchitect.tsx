...
import { Palette, Upload, Loader2, Zap, Download, ShieldCheck, Info, Camera, Trash2, Check, Target, HelpCircle, BookOpen, Layout, Sparkles } from 'lucide-react';
...
export const HairColorArchitect: React.FC = () => {
  ...
  const toolData = t.tools['hair-architect'] || {
    name: 'Hair Color Architect',
  ...
  const transmuteHair = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: {
          parts: [
            ...
            { text: `Identify the person's hair in this image. Change the hair color to exactly ${targetColor.name} (Hex: ${targetColor.hex}). Maintain natural textures, lighting, and reflections. Return the modified image only. Use numerals 0123456789` }
          ]
        }
      });
      ...
      console.error(err);
      alert("Neural desync: Failed to map hair follicles 0123456789");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-12 animate-in fade-in duration-1000" dir="ltr">
      ...
      <div className="w-full h-24 md:h-32 bg-black/[0.02] border border-dashed border-[#e7d8c5] rounded-3xl flex items-center justify-center overflow-hidden">
         <div className="text-center">
            <p className="text-[8px] font-black uppercase tracking-[0.6em] text-[#b7a896] italic">Responsive Pigment Archive Ad</p>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[9px] font-bold text-[#D4AF37]/40 uppercase tracking-[0.4em]">Neural Follicle Transmutation 0123456789</p>
            </div>
          </div>
          <div className="flex items-center gap-2 px-5 py-2 bg-emerald-500/5 border border-emerald-500/20 rounded-xl">
             <ShieldCheck size={14} className="text-emerald-400" />
             <span className="text-[9px] font-black uppercase tracking-widest text-emerald-400/60 tabular-nums">Follicle Logic Verified 0123456789</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          <div className="lg:col-span-5 space-y-8">
            <div className="space-y-4">
              <label className="text-[10px] font-black uppercase tracking-[0.4em] text-gray-500 italic px-2">1. Ingest Portrait Asset</label>
              <div 
                onClick={() => fileInputRef.current?.click()}
                className={`h-64 bg-black border-2 border-dashed rounded-[2.5rem] flex flex-col items-center justify-center cursor-pointer transition-all overflow-hidden ${preview ? 'border-pink-500/40' : 'border-white/5 hover:border-pink-500/40'}`}
              >
            ...
            <div className="space-y-4">
               <label className="text-[10px] font-black uppercase tracking-[0.4em] text-gray-500 italic px-2">2. Select Target Pigment 0123456789</label>
            ...
            <button onClick={transmuteHair} disabled={loading || !file} className="w-full bg-pink-600 text-white py-6 rounded-[2.5rem] font-black text-xl uppercase tracking-tighter italic flex items-center justify-center gap-4 hover:scale-[1.02] active:scale-95 transition-all shadow-[0_20px_50px_rgba(219,39,119,0.3)] disabled:opacity-20">
              {loading ? <Loader2 className="animate-spin" size={24} /> : <Zap size={24} />}
              Execute Pigment Shift
            </button>
          </div>

          <div className="lg:col-span-7 flex flex-col h-full space-y-6">
            <div className="flex justify-between items-center px-2">
              <label className="text-[10px] font-black uppercase tracking-[0.4em] text-pink-500 italic">Neural Result Viewport</label>
            ...
            <div className="relative flex-grow bg-black border border-white/5 rounded-[3.5rem] p-6 flex items-center justify-center min-h-[450px] overflow-hidden group shadow-inner">
               ...
               {loading ? (
                 <div className="text-center space-y-4 animate-pulse">
                    ...
                    <p className="text-[10px] font-black uppercase tracking-[0.5em] text-pink-500">Mapping Hair Follicles 0123456789...</p>
                 </div>
               ) : result ? (
                 <div className="relative z-10 w-full h-full flex flex-col items-center justify-center animate-in fade-in zoom-in duration-700">
                    ...
                    <button onClick={() => { const a = document.createElement('a'); a.href = result; a.download = 'Aureate_Hair_Shift.png'; a.click(); }} className="mt-6 flex items-center gap-3 px-8 py-3 bg-pink-600 text-white rounded-xl font-black text-xs uppercase tracking-widest hover:scale-105 transition-all shadow-xl">
      ...
      <div className="w-full h-32 md:h-64 bg-black/[0.03] border-4 border-dashed border-[#e7d8c5] rounded-[4rem] flex flex-col items-center justify-center p-8 opacity-40">
         ...
         <p className="text-[10px] font-black uppercase tracking-[0.8em] text-[#b7a896] italic text-center">Institutional Pigment Display Ad</p>
