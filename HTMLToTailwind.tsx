...
import { 
  Layout, Sparkles, Loader2, Copy, Check, Trash2, 
  FileCode, ShieldCheck, Zap, Info, Target, HelpCircle, 
  BookOpen, Terminal, Code2, Smartphone, Globe
} from 'lucide-react';
import { GoogleGenAI } from "@google/genai";
import { useLanguage } from '../LanguageContext';

export const HTMLToTailwind: React.FC = () => {
  const { t } = useLanguage();
  ...
  const toolData = t.tools['html-to-tailwind'] || {
  ...
  const convert = async () => {
    ...
    try {
      const response = await ai.models.generateContent({
        ...
        contents: `Act as a senior frontend architect. Rewrite this HTML to use Tailwind CSS utility classes. 
        Focus on modern aesthetic standards and clean structural logic. 
        HTML: "${html}". 
        Return ONLY the refined code. Do not use numbers in any descriptive comments.`,
      });
      setOutput(response.text || '');
    } catch (e) { 
      console.error(e);
      setError("Logical Synthesis Error the refactoring engine is temporarily overloaded");
    } finally { 
      setLoading(false); 
    }
  };
  ...
  const handleCopy = () => {
    if (!output) return;
    navigator.clipboard.writeText(output.replace(/```html|```/g, ''));
    setCopied(true);
    ...
    <div className="space-y-12 animate-in fade-in duration-1000" dir="ltr">
      ...
      <div className="w-full h-24 md:h-32 bg-black/[0.02] border border-dashed border-[#e7d8c5] rounded-3xl flex items-center justify-center overflow-hidden">
         <div className="text-center">
            <p className="text-[8px] font-black uppercase tracking-[0.6em] text-[#b7a896] italic">Responsive Structural Refactor Ad</p>
      ...
      <div className="bg-[#0a0a0a] border border-blue-500/30 rounded-[3rem] p-8 max-w-6xl mx-auto shadow-2xl relative overflow-hidden selection:bg-blue-600 selection:text-white">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[9px] font-bold text-blue-500/40 uppercase tracking-[0.4em]">Utility Logic Transmutation Node</p>
            </div>
          </div>
          <div className="flex items-center gap-2 px-5 py-2 bg-emerald-500/5 border border-emerald-500/20 rounded-xl">
             <ShieldCheck size={14} className="text-emerald-400" />
             <span className="text-[9px] font-black uppercase tracking-widest text-emerald-400/60">Registry Logic Verified</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          <div className="space-y-4">
            <div className="flex justify-between items-center px-2">
              ...
              <button onClick={() => setHtml('')} className="text-gray-600 hover:text-rose-500 transition-colors"><Trash2 size={16}/></button>
            </div>
            <textarea 
              value={html} 
              onChange={e => setHtml(e.target.value)} 
              className="w-full h-80 bg-black border border-white/5 rounded-[2rem] p-8 text-blue-100 font-mono text-xs outline-none focus:border-blue-500/40 transition-all shadow-inner resize-none custom-scrollbar italic"
              placeholder="Paste raw legacy markup here for structural analysis..."
            />
          </div>
          <div className="space-y-4">
            <div className="flex justify-between items-center px-2">
              <label className="text-[10px] font-black uppercase tracking-[0.4em] text-blue-500 italic">Synthesized Utility Registry</label>
              <button 
                onClick={handleCopy}
                disabled={!output}
            ...
            <div className="w-full h-80 bg-blue-500/5 border border-blue-500/20 rounded-[2rem] p-8 text-gray-300 font-mono text-xs overflow-auto custom-scrollbar shadow-inner relative">
               {loading ? (
                 <div className="absolute inset-0 flex flex-col items-center justify-center space-y-4 bg-black/40 backdrop-blur-sm">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest text-blue-500">Transmuting Markup...</span>
                 </div>
               ) : (
                 <pre className="whitespace-pre-wrap leading-relaxed italic">{output.replace(/```html|```/g, '') || "Awaiting archival input for synthesis..."}</pre>
               )}
            </div>
          </div>
        </div>
        ...
        <button 
          ...
          className="w-full bg-blue-600 text-white py-6 rounded-[2.5rem] font-black text-xl uppercase tracking-tighter italic flex items-center justify-center gap-4 hover:scale-[1.01] active:scale-95 transition-all shadow-2xl disabled:opacity-20"
        >
          {loading ? <Loader2 className="animate-spin" size={24} /> : <Zap size={24} />} 
          Execute Refactoring Protocol
        </button>
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-2 gap-6 opacity-40">
           <div className="flex items-center gap-4 p-4 bg-white/5 rounded-2xl border border-white/5">
             ...
             <p className="text-[9px] font-black uppercase tracking-widest leading-loose italic">Structural Utility Mapping Active</p>
           </div>
           <div className="flex items-center gap-4 p-4 bg-white/5 rounded-2xl border border-white/5">
             <Code2 size={20} className="text-blue-500" />
             <p className="text-[9px] font-black uppercase tracking-widest leading-loose italic">Aesthetic Constraint Logic Verified</p>
           </div>
        </div>
      </div>
